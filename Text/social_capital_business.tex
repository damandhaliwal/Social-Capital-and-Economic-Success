...
Our identification strategy involves a difference-in-differences (DiD) approach with heterogeneous treatment intensity. The key identifying assumption is parallel trends: without the pandemic, firms in high- and low- social capital counties would have followed similar survival trajectories. We provide suggestive evidence for this assumption using pre-period data (2016-2019). 

\subsection{Key Variables}
The primary outcome variables of interest are firm survival, a binary indicator equal to 1 if a firm present in 2019 remains in the Data Axle database in 2024, and sales growth, the log change in sales from 2019 to 2024, conditional on survival.

The treatment variable is the Economic Connectedness (EC) component of the social capital index. We focus on EC as it captures cross-class interactions, which are hypothesized to be particularly important for economic resilience. \cite{chetty_social_2022-1} proved that cohesion and civic engagement did not have an effect on income mobility, however, we will include them in robustness checks.

The control variables include log employees in 2019, log sales in 2019, state-fixed effects and two-digit NAICS industry fixed effects.
...
\begin{equation}
    Survival_{i,2024} = \alpha + \beta \cdot SC_{c} + \gamma \cdot ln(Sales_{i,2019})  + \theta_s + \mu_j + \epsilon_{ic}
\end{equation}
where $Survival_{i,2024}$ is a binary indicator for whether firm $i$ in county $c$ survived until 2024, $SC \in \{EC_c, COH_c, CIV_c\}$ and $EC_c$, $COH_c$, and $CIV_c$ are the Economic Connectedness, Social Cohesion, and Civic Engagement components of the social capital index for county $c$, respectively. $ln(Sales_{i,2019})$ is the log of sales for firm $i$ in 2019, $\theta_s$ are state fixed effects, $\mu_j$ are two-digit NAICS industry fixed effects, and $\epsilon_{ic}$ is the error term. Here $i$ indexes firm, $c$ indexes county, $s$ indexes state, and $j$ indexes industry. Standard errors are clustered at the county level to account for potential correlation of errors within counties.
...
We implment the Partially Linear Regression (PLR) DML estimator for each $SC \in \{EC_c, COH_c, CIV_c\}$ as follows:
\begin{equation}
    Y_i = \theta_{SC} \cdot SC_c + g(X_i) + \epsilon_i
\end{equation}
\begin{equation}
    SC_c = m(X_i) + \nu_i
\end{equation}
where $Y_i$ is the firm survival, $SC_i$ is the social capital measure, $X_i$ is a vector of control variables (log sales in 2019, state-fixed effects, two-digit NAICS industry fixed effects), and $g(\cdot)$ and $m(\cdot)$ are unknown functions estimated using machine learning methods. 
...
\begin{equation}
    Y_{ij} = \theta_{SC,j} \cdot SC_c + g_j(X_{ij}) + \epsilon_{ij}
\end{equation}
where $j$ indexes industry or sub-industry. This allows us to estimate industry-specific treatment effects $\theta_{SC,j}$ while flexibly controlling for confounders within each industry.
...
We hypothesize that social capital may amplify the performance of already successful firms while providing limited benefits to struggling firms due to word of mouth sales. Quantile regression will help us test this hypothesis by examining how the coefficients on social capital measures vary across different quantiles of the firm performance distribution.
